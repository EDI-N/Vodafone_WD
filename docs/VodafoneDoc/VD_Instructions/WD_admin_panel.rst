Налаштування панелі адміністратора VodafoneDoc
###################################################################
...
.. hint::
   Нові користувачі можуть самостійно пройти "Реєстрацію" і долучитись до сервісу (детальніше в нашій `інструкції <https://vodafone-wd-wiki.edin.ua/uk/latest/VodafoneDoc/VD_registration.html>`__).

----------------------------------------------------

Для проведення адміністративних налаштувань на рівні акаунта потрібно клікнути лівою кнопкою мишки на іконку користувача та обрати **"Панель адміністратора"** (`налаштування профілю користувача <https://vodafone-wd-wiki.edin.ua/uk/latest/VodafoneDoc/VD_Instructions/VD_user_profile.html>`__ описані в окремій інструкції):
...
В сервісі Vodafone Doc є два типи акаунтів (тип вибирається при його створенні): **особистий акаунт** і **акаунт компанії**, що в налаштуваннях відрізняються наявністю у останнього поля для "Інформація про компанію":
...
Конфігурація доменів дозволяє керувати власними доменанами, активувати та деактивувати домени всередині сервісу. Домени необхідні для додавання, перевірки чи створення користувачів на платформі з доменом, яким Ви володієте. Тобто всі користувачі сервісу VodafoneDoc, що мають сказаний на цій сторінці поштовий домен автоматично підв'язуються під цей акаунт.
...
Зміни додані до DNS налаштувань можуть зайняти деякий час. Як тільки сервіс VodafoneDocEdin знайде текстовий запис в DNS налаштуваннях домену, він стане верифікованим у системі (змінить свій статус).
...
Якщо користувач, якого запросили уже користується сервісом, то йому згідно його налаштувань (ПОСИЛАННЯ) в сервіс Vodafone Doc надійде повідомлення та email, а саме запрошення буде відображатись в Інформації профілю (вкладка `Запрошення <https://vodafone-wd-wiki.edin.ua/uk/latest/VodafoneDoc/VD_Instructions/VD_user_profile.html#invite>`__):
...
В блоці "Права доступу для ролі" потрібно обрати дозволи, що буде включати майбутня роль (всі дозволи детально описані в наступних підрозділах за посиланнями: `"Дозволи акаунта" <https://vodafone-wd-wiki.edin.ua/uk/latest/VodafoneDoc/VD_Instructions/WD_admin_panel.html#account-permissions>`__ та `"Дозволи мейлбокса" <https://vodafone-wd-wiki.edin.ua/uk/latest/VodafoneDoc/VD_Instructions/WD_admin_panel.html#mailbox-permissions>`__). Дозволи розділені на групи і для того аби відкрити/закрити список дозволів, що налічує група можливо скористатись кнопками |arrowD| / |arrowU| відповідно або відкрити/закрити всі групи за допомогою кнопки **"Показати все"**:
...
.. attention::
   Видалений мейлбокс не можливо відновити. Мейлбокс зникає зі списку і якщо користувач спробує використати видалений мейлбокс, то він отримає 410 помилку і буде перенаправлений на інший мейлбокс, до якого він має доступ. Якщо користувач не має доступу до будь-якого іншого мейлбокса, то він буде переспрямований у `профіль користувача <https://vodafone-wd-wiki.edin.ua/uk/latest/VodafoneDoc/VD_Instructions/VD_user_profile.html>`__. 

**5 Інтеграції**
================================================

У розділі **Інтеграції** представлені налаштування інтеграції з сервісом Vodafone Doc за такими протоколами передачі даних, як SFTP, AS2, SOAP. Як користувач цієї платформи ви вже можете використовувати інтеграцію через `API <https://wd-api.edin.ua/swagger-ui/#/>`__. `API <https://wd-api.edin.ua/swagger-ui/#/>`__ доступна за замовчуванням і може використовуватись разом з іншими інтеграціями.
...
.. image:: pics_WD_admin_panel/WD_admin_panel_079.png
   :align: center
...
.. attention::
   **"Деталі підключення"** (3) генеруються автоматично відповідно до протоколу з'єднання (`детальніше <https://vodafone-wd-wiki.edin.ua/uk/latest/VodafoneDoc/VD_Instructions/WD_admin_panel.html#ftp>`__) і є унікальними для кожного мейлбокса. Клієнт може скопіювати кожен з цих параметрів, натиснувши кнопку копіювання. Клієнт не може змінювати будь-які параметри, крім пароля. Для зміни пароля клієнт повинен натиснути кнопку "оновити пароль". 
...
Правило конвертації дозволяє перетворювати документ із формату замовника (eancom/xml) у формат Vodafone Doc і може здійснювати зворотне конвертації. Для цього клієнт повинен створити правило конвертації і в залежності від його вибору на цій сторінці він може створити правило конвертації для вхідних та вихідних документів. Крім того, якщо клієнт відкрив існуюче правило конвертації, він може оновити, видалити або клонувати це правило.
...
.. image:: pics_WD_admin_panel/WD_admin_panel_082.png
   :align: center
...
.. code:: xml
   ...
   <xsl:stylesheet version="1.0"
                   ...
                   xmlns:wdExtensions="java:com.VodafoneDoc.xslt.extensions.WdExtensions"
...
**Примітки щодо використання:** Клієнт може використовувати різні формати обміну даними. Якщо клієнт використовував формат обміну даними VodafoneDoc, він повинен використовувати **/inbox** та **/outbox** як робочі папки, якщо він використовував інший формат, то робоча папка має бути **/cinbox** та **/coutbox**. 
